    ...
    if (selectedEmployee && selectedDate && selectedService && businessHours.length > 0) {
      const fetchSlots = async () => {
        ...
        try {
          ...
          const occupied = await getOccupiedSlots(Number(selectedEmployee.id), format(selectedDate, "yyyy-MM-dd"));
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      const msg = typeof err.response?.data === 'string'
        ? err.response.data
        : (err.response?.data as { message?: string })?.message || "Error al solicitar la reserva";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        {selectedSlot && (
          ...
          >
            ...
            >
              {userId ? `RESERVAR A LAS ${selectedSlot}` : "Siguiente"}
