...
export const ServiceView = () => {
  ...
  const handleDelete = (id: string) => {
    axios
      ...
      .then(() => setServices((prev) => prev.filter((service) => service.id !== id)))
      ...
      <div className={styles.container}>
        ...
        {services.map((service) => (
          <div key={service.id} className={styles.itemList}>
            ...
            <input type="text" placeholder={`${service.duration} min`} readOnly />
            <input type="text" placeholder={`$${service.price}`} readOnly />
            <button onClick={() => navigate(`/services/edit/${service.id}`)}>Editar</button>
            <button onClick={() => handleDelete(service.id)}>Eliminar</button>
          </div>
        ))}
        <div className={styles.addServiceContainer}>
          <button className={styles.addButton} onClick={() => navigate("/services/new")}>+</button>
        </div>
      </div>
      <div className={styles.containerButtons}>
        <button style={{ backgroundColor: "#295366" }} onClick={() => navigate("/")}>Volver</button>
        <button style={{ backgroundColor: "#F96E2A" }} onClick={() => navigate("/homeclient")}>Aceptar y Guardar</button>
      </div>
    </section>
  );
};
